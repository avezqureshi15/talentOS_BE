...
@router.get("/summary", response_model=SlotSummaryResponse, dependencies=[Depends(require_permission(Permission.SLOT_VIEW_ALL))])
def get_slots_summary(
    db: Session = Depends(get_db),
    current_user: UserInfo = Depends(get_current_user),
):
    service = SlotService(db)
    return service.get_summary(_tenant_id_for_user(current_user))
...
def get_slots_for_employee(
    ...
    service = SlotService(db)
    return service.get_slots_for_employee(employee_id, detail=detail)
...
def update_slot_status(
    ...
    service = SlotService(db)
    return service.update_slot_status(slot_id, data)
