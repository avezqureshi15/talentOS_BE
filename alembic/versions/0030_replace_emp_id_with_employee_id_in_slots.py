...
def upgrade() -> None:
    ...
    # 2. Backfill employee_id from users table using existing emp_id mapping
    op.execute(
        """
        UPDATE slots
        SET employee_id = users.id
        FROM users
        WHERE slots.emp_id = users.emp_id
        """
    )

    # 3. Drop the old FK and index on emp_id
    op.drop_constraint("fk_slots_emp_id_users", "slots", type_="foreignkey")
    op.drop_index("ix_slots_emp_id", table_name="slots")

    # 4. Drop the emp_id column
    op.drop_column("slots", "emp_id")

    # 5. Make employee_id NOT NULL
    op.alter_column("slots", "employee_id", nullable=False)

    # 6. Add FK and index on employee_id
